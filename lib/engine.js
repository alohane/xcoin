...
module.exports = function (s, conf, core) {
  ...
  if (_.isUndefined(s.exchange)) {
    try {
      if (so.mode !== "live") {
        // console.log("hihi test not live in engine.js");
        ...
        // console.log(s.exchange.getDepth());
      ...
      s.exchange.getBalance(
        ...
        function (err, balance) {
          ...
          if (exitSymbols.length) {
            s.exchange.getTickers({ exitSymbols }, function (err, realTickers) {
              ...
              if (realTickers) {
                ...
                // console.log('getTickers', symbols)
  ...
  function initKLinePeriod(symbol, trade) {
    ...
    if (trade.isTrade) {
      trade.period_id = tb(trade.time)
        ...
        .toString();
      // console.log('\nget trade'.cyan, trade.period_id.cyan, moment(trade.time).format('MMDD HH:mm:ss').green)
      const inPeriod =
        s.symbols[symbol.product_id].period &&
